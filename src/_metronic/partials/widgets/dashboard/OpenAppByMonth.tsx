...
import {getopenappcount} from './core/_requests'
...
const OpenAppByMonth: React.FC<Props> = ({className}) => {
  const [openAppCount, setOpenAppCount] = useState<Array<IdCreatedAtModel>>([]);
  const [totalOpenAppCount, setTotalOpenAppCount] = useState<number>(0);
  ...
  useEffect(() => {
    ...
    if (!openAppCount || !openAppCount.length) {
      return
    }

    var date = new Date(openAppCount[0].createdAt);
    var countArray = [];
    var dayArray = [];
    var maxCount = 0;
    ...
    for (var dy = 1 ; dy <= lastDay; dy++){
      ...
      let count = 0;
      openAppCount.map((eData: IdCreatedAtModel, index: number) => {
    ...
    <div className={`card ${className}`}>
      <div className="card-header py-5">
        <h3 className="card-title align-items-start flex-column">
          <span className="card-label fw-bolder text-dark">Average app’s opening per day</span>
        </h3>
      </div>
      <div className="card-body d-flex justify-content-between flex-column pb-1 px-0">
        <div className="px-9 mb-5">
          <div className="d-flex mb-2">
            <span className="fs-2hx fw-bolder text-gray-800 me-2 lh-1 ls-n2">{totalOpenAppCount.toLocaleString()}</span>
          </div>
        </div>
        <div
          id='chartContainer'

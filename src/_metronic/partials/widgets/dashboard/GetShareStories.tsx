...
import {getsharestories} from './core/_requests'
...
const GetShareStories: React.FC<Props> = ({className}) => {
  const [shareStories, setShareStories] = useState<Array<IdCreatedAtModel>>([]);
  const [totalShareStories, setTotalShareStories] = useState<number>(0);
  ...
  useEffect(() => {
    ...
    if (!shareStories || !shareStories.length) {
      return
    }

    var date = new Date(shareStories[0].createdAt);
    var countArray = [];
    var dayArray = [];
    var maxCount = 0;
    ...
    for (var dy = 1 ; dy <= lastDay; dy++){
      ...
      let count = 0;
      shareStories.map((eData: IdCreatedAtModel, index: number) => {
    ...
    <div className={`card ${className}`}>
      <div className="card-header py-5">
        <h3 className="card-title align-items-start flex-column">
          <span className="card-label fw-bolder text-dark">Number of time people share stories in other social network</span>
        </h3>
      </div>
      <div className="card-body d-flex justify-content-between flex-column pb-1 px-0">
        <div className="px-9 mb-5">
          <div className="d-flex mb-2">
            <span className="fs-2hx fw-bolder text-gray-800 me-2 lh-1 ls-n2">{totalShareStories.toLocaleString()}</span>
          </div>
        </div>
        <div
          id='chartContainer'
